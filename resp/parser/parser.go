...
func parse0(reader io.Reader, ch chan<- *Payload) {
	defer func() {
		if err := recover(); err != nil {
			...
			if reason == "close of closed channel" {
				return
			}

			logger.Error(string(debug.Stack()))
		} else {
			logger.Error("notice : why programme exited without err ?")
		}
		go parse0(reader, ch)
	}()
	...
	for {
		...
		if !state.readingMultiLine {
			...
			if msg[0] == '*' {
				...
				if err != nil {
					ch <- &Payload{
						Err: errors.New("-Err Protocol error: " + string(msg)),
					}
			...
			if state.finished() {
				...
				if state.msgType == '*' || state.msgType == '$' {
...
func readLine(bufReader *bufio.Reader, state *readState) ([]byte, bool, error) {
	...
	if state.bulkLen == 0 { // read normal line
		...
		if len(msg) == 0 || msg[len(msg)-2] != '\r' {
			return nil, false, errors.New("-Err protocol error: " + string(msg))
		}
...
func parseMultiBulkHeader(msg []byte, state *readState) error {
	...
	if err != nil {
		return errors.New("-Err protocol error: " + string(msg))
	}
	if expectedLine == 0 {
...
func parseSingleLineReply(msg []byte) (resp.Reply, error) {
	...
	case ':': // int reply
		...
		if err != nil {
			return nil, errors.New("-Err protocol error: " + string(msg))
		}
		result = reply.MakeIntReply(val)
	default:
		result = reply.MakeProtocolErrReply(str)
	}
	return result, nil
}
